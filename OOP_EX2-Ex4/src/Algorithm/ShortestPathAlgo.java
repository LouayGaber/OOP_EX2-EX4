...
public class ShortestPathAlgo {
	...
	public ShortestPathAlgo(Game theGame) {	
		...
		ArrayList<Fruit> clone = new ArrayList<Fruit>(theGame.Fruits.size());  for (Fruit item : theGame.Fruits) clone.add(item);
		this.fruits = clone;	//Create a new fruit for not to overwrite Game data later
		this.Packmans = theGame.Packmans;
	}
	...
	public Path algoSinglePackman(Packman oriPackman){
		...
		Tempfruits=oriPackman.getPath().getmyTarget();

		Packman tempPackman = new Packman(oriPackman.getLocation(),oriPackman.getSpeed(),oriPackman.getRadius());
		Path Dis = disAlgo(tempPackman,Tempfruits);
		Path p = calFastDisOnePack(tempPackman, Tempfruits);
		
		
		double timeFor1 = Dis.TotalTime(tempPackman);
		
		double timeFor2 = p.TotalTime(tempPackman);


		if(timeFor1 < timeFor2) {
			Dis.setTime(timeFor1);
	...
	public Path calFastDisOnePack (Packman packman, ArrayList<Fruit> myFurits) {
		...
		packman.getPath().getmyTarget().add(theCloserTemp);
		packman.setLocation(theCloserTemp.getFlocation());
	...
	public Path disAlgo(Packman packman, ArrayList<Fruit> myFurits) {
		...
		for (int i = 0; i < myFurits.size(); i++) {
			double temp = themap.Distance_in_Pixel(packman.getLocation(), myFurits.get(i).getFlocation());
		...
		for(double distance : SortPathByDis) {
			...
			for (int j = 0; j < myFurits.size(); j++) {
				temp = themap.Distance_in_Pixel(packman.getLocation(), myFurits.get(j).getFlocation());

				if(temp == distance) {
					ans.getmyTarget().add(myFurits.get(j));
					break;
				}
	...
	public ArrayList<Packman> algoMultiPackmans (){
		...
		for (int i = 0; i < myPackmens.size(); i++) {
			Packman p = new Packman(myPackmens.get(i).getLocation(),myPackmens.get(i).getSpeed(), myPackmens.get(i).getRadius());
			ans.add(p);
		}
		myPackmens = Algomulti(myPackmens,myFurits);

		for (int i = 0; i < myPackmens.size(); i++) {
			myPackmens.get(i).setLocation(ans.get(i).getLocation());
		}
		...
		double longestTime = myPath.TotalTime(myPackmens.get(0));
		double temp = 0;

		for (int i = 1; i < myPackmens.size(); i++) {
			temp = myPath.TotalTime(myPackmens.get(i));
			if (temp > longestTime) {
				longestTime = temp;
			}
		}
	...
	private ArrayList<Packman> Algomulti (ArrayList<Packman> myPackmans , ArrayList<Fruit>myFurits) {
		...
		double FastTime = myPath.Time2Target(myPackmans.get(0),theCloserFurit);
		...
		for (int i = 0; i < myPackmans.size(); i++) {
			...
			tempTime = myPath.Time2Target(myPackmans.get(i),tempFruit);
		...
		thePackman.getPath().getmyTarget().add(theCloserFurit);
		thePackman.setLocation(theCloserFurit.getFlocation());
	...
	double FastSpeedToFriut(Packman packman ,ArrayList<Fruit> myFurits ) {
		Path p = new Path();
		double fastTime = p.Time2Target(packman, myFurits.get(0));
		double tempTime = 0;

		for (int i = 1; i < myFurits.size(); i++) {

			tempTime = p.Time2Target(packman, myFurits.get(i));
