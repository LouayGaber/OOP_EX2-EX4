...
public class Myframe extends JFrame implements MouseListener , KeyListener
{
	...
	private void initGUI() {
		...
		try {	myImage = ImageIO.read(new File("C:\\Users\\Louay\\git\\OOP_EX2-EX4\\data\\Ariel1.png")); } catch (IOException e) { e.printStackTrace();	}	
		try {	packimage = ImageIO.read(new File("C:\\Users\\Louay\\git\\OOP_EX2-EX4\\data\\Packman.png")); } catch (IOException e) { e.printStackTrace();	}
		try {	Fruitimage = ImageIO.read(new File("C:\\\\Users\\\\Louay\\\\git\\\\OOP_EX2-EX4\\\\data\\\\fruit.png")); } catch (IOException e) { e.printStackTrace();	}
		try {	box = ImageIO.read(new File("")); } catch (IOException e) { e.printStackTrace();	}
		try {	ghost = ImageIO.read(new File("")); } catch (IOException e) { e.printStackTrace();	}
		try {	player = ImageIO.read(new File("C:\\\\Users\\\\Louay\\\\git\\\\OOP_EX2-EX4\\\\data\\\\user.png")); } catch (IOException e) { e.printStackTrace();	}
		...
		Csv_read.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (returnValue == JFileChooser.APPROVE_OPTION) {
					...
					isGamer = 4;
					myGame.setPath(fileChooser.getSelectedFile().getPath());
					repaint();
		...
		Csv_writing_csv.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (returnValue == JFileChooser.APPROVE_OPTION) {
					...
					myGame.setPath(selectedFile.getAbsolutePath());
					try {
						myGame.saveGame();
		...
		if (isGamer!=0) {
			...
			if(myGame.Fruits.size() > 0) {
				...
				for (int i=0; i<myGame.Fruits.size(); i++) 	{
					x1=(int)(myGame.Fruits.get(i).getFlocation().x()*getWidth());
					y1=(int)(myGame.Fruits.get(i).getFlocation().y()*getHeight());	
			...
			for (int j=0; j<myGame.Packmans.size(); j++) {
				...
				x1=(myGame.Packmans.get(j).getLocation().x()*getWidth());
				y1=(myGame.Packmans.get(j).getLocation().y()*getHeight());	
			...
			if(myGame.Player_user!=null){
				...
				myGame.Player_user.nextPoint(dir);//mygame_playuser
